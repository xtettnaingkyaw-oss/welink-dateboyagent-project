...
၂။ frontend-web/src/pages/Admin.jsx ကို အောက်ပါအတိုင်း အစားထိုးပါ-
ဒီဖိုင်မှာ handleApprove နဲ့ handleDeleteDateBoy နှိပ်လိုက်တဲ့အခါ User ဆီကို Telegram Notification လှမ်းပို့မယ့် fetch စနစ် ပေါင်းထည့်ထားပါတယ်။
...
import { UserCheck, Clock, Plus, Trash2, CheckCircle2, Settings, Eye, Pencil, EyeOff, Save, X, CreditCard, FileText } from 'lucide-react';
...
export default function Admin() {
  ...
  const [appConfig, setAppConfig] = useState({
    paymentInfo: 'KPay: 09123456789 (Name)', privFee: 5000, feeSec: 30000, feeDay: 70000, feeNight: 100000,
    reqText: '၁။ အသက် ၂၁ နှစ်ပြည့်ပြီးသူ ဖြစ်ရပါမည်။\n၂။ ကိုယ်အမူအရာ သန့်ရှင်းသပ်ရပ်ရမည်။',
    ruleText: '၁။ အမှန်တကယ် လုပ်ကိုင်လိုသူ ဖြစ်ရပါမည်။\n၂။ ဖောက်သည်များအား ယဉ်ကျေးစွာ ဆက်ဆံရမည်။'
  });
  ...
  // 🔔 Web App မှ အတည်ပြုလျှင် User သို့ Notify ပို့ခြင်း
  const handleApprove = async (id) => {
    ...
    if (boy && boy.telegramChatId) {
      fetch('/api/webhook', {
        ...
        body: JSON.stringify({ internal_action: 'notify_user', chatId: boy.telegramChatId, text: `🎉 ဝမ်းသာပါတယ် ခင်ဗျာ! သင့်ရဲ့ Date Boy လျှောက်လွှာကို Admin မှ အတည်ပြုပေးလိုက်ပါပြီ။`, useMenu: true })
      }).catch(e => console.error(e));
    }
  };

  // 🔔 Web App မှ ပယ်ချလျှင် User သို့ Notify ပို့ခြင်း
  const handleDeleteDateBoy = async (id) => {
    if (window.confirm('ဖျက်ပစ်မှာ သေချာပါသလား?')) {
      ...
      if (boy && boy.status === 'pending' && boy.telegramChatId) {
        fetch('/api/webhook', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ internal_action: 'notify_user', chatId: boy.telegramChatId, text: `❌ ဝမ်းနည်းပါတယ် ခင်ဗျာ။ သင့်ရဲ့ Date Boy လျှောက်လွှာကို ပယ်ချလိုက်ပါသည်။` })
        }).catch(e => console.error(e));
      }
