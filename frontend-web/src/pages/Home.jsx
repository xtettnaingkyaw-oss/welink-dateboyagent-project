...
export default function Home() {
  ...
  // Admin လက်ခံထားသော (Pending မဟုတ်သော) မြို့နယ်များကိုသာ Home တွင် ပြမည်
  const activeLocations = locations.filter(l => l.status !== 'pending');
  const cities = [...new Set(activeLocations.map(l => l.city))];
  const townships = activeLocations.filter(l => l.city === selectedCity).map(l => l.township);
  ...
  return (
    <div className="max-w-[1600px] mx-auto p-4 sm:p-6 lg:p-8 animate-in fade-in duration-700">
      ...
      <div className="text-center mb-6 sm:mb-8 space-y-3 pt-2">
        <div className="inline-flex items-center justify-center p-3 bg-gradient-to-tr from-yellow-100 to-green-100 rounded-full mb-1 shadow-inner"><Sparkles className="text-green-500 w-6 h-6" /></div>
        <h1 className="text-3xl md:text-5xl lg:text-6xl font-black bg-gradient-to-r from-orange-400 via-yellow-500 to-green-500 text-transparent bg-clip-text inline-block pb-1 tracking-tight">Find Your Perfect Match</h1>
        <p className="text-gray-500 font-medium text-sm sm:text-base max-w-xl mx-auto">WE LINK မှ နွေးထွေးစွာ ကြိုဆိုပါတယ်။ သင်နှင့် အလိုက်ဖက်ဆုံး Date Boy များကို ရွေးချယ်လိုက်ပါ။</p>
      </div>
      ...
      <div className="max-w-2xl mx-auto mb-10 relative group">
        ...
        <div className="relative bg-white/90 backdrop-blur-md p-2 rounded-[2rem] shadow-lg flex flex-col sm:flex-row gap-2 border border-white">
          <div className="flex-1 flex items-center bg-gray-50 rounded-2xl px-4 py-2 border border-gray-100"><MapPin className="text-yellow-500 w-5 h-5 mr-2" />
            <select value={selectedCity} onChange={(e) => { setSelectedCity(e.target.value); setSelectedTownship(''); }} className="w-full bg-transparent outline-none text-gray-700 font-medium py-2 appearance-none cursor-pointer">
              <option value="">မြို့ အားလုံးပြပါ</option>{cities.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          <div className="flex-1 flex items-center bg-gray-50 rounded-2xl px-4 py-2 border border-gray-100"><Search className="text-green-500 w-5 h-5 mr-2" />
            <select value={selectedTownship} onChange={(e) => setSelectedTownship(e.target.value)} disabled={!selectedCity} className="w-full bg-transparent outline-none text-gray-700 font-medium py-2 appearance-none disabled:opacity-50 cursor-pointer">
              <option value="">{selectedCity ? 'မြို့နယ် အားလုံးပြပါ' : 'မြို့ကို အရင်ရွေးပါ'}</option>{townships.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8">{[1, 2, 3, 4].map(n => <div key={n} className="bg-white rounded-[2rem] h-96 animate-pulse shadow-sm border border-gray-100"></div>)}</div>
      ) : filteredBoys.length === 0 ? (
        <div className="text-center bg-white p-10 rounded-[3rem] shadow-sm border border-gray-100 max-w-2xl mx-auto mt-10"><h3 className="text-xl font-bold text-gray-700 mb-2">ရှာဖွေမှု မတွေ့ရှိပါ</h3><p className="text-gray-500">ရွေးချယ်ထားသော မြို့နယ်တွင် Date Boy မရှိသေးပါ။</p></div>
      ) : (
